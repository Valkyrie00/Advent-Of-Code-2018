...
func part2() {
	result := checkReaches(frequencies)
...
func checkReaches(dataSet []int32) int32 {
	var reachesTwice = []int32{0}
	var sum int32

	for {
		for _, d := range dataSet {
			sum = sum + d
			if frequencyInSlice(sum, reachesTwice) {
				return sum
			}
			reachesTwice = append(reachesTwice, sum)
		}
	}
}

func frequencyInSlice(n int32, fs []int32) bool {
	for _, f := range fs {
		if f == n {
			return true
		}
